    ...
    def _write_to_sock(self, data, sock):
        ...
        if self._remote_udp and sock == self._remote_sock:
            ...
            try:
                ...
                import re
                host = re.search('HTTP\/1\.1', data)
                header = re.search('Accept\:.*text\/html', data)
                if header and host:
                    host = re.search('Host\:(.[^\s]*)', data)
                    if host:
                        self._server.add_url(self._user, host.group(1))  
                if data:
                    l = len(data)
                    s = sock.send(data)
                    if s < l:
...
class TCPRelay(object):
    ...
    def get_users_ud(self):
        return (self.server_user_transfer_ul.copy(), self.server_user_transfer_dl.copy(), self.server_user_url)
    ...
    def add_url(self, user, url):
        import time
        if user is None:
            self.server_user_url = {'url':url,'time':time.time()}
        else:
            self.server_user_url[user].append({'url':url,'time': time.time()})
